...
contract ClaimLinkEscrow is EIP712, ReentrancyGuard {
    ...
    function createClaimLink(address claimSigner, string calldata message) external payable returns (bytes32 linkId) {
        return _createClaimLink(claimSigner, message);
    }

    function _createClaimLink(address claimSigner, string memory message) internal returns (bytes32 linkId) {
    ...
    function getMessage(bytes32 linkId) external view returns (string memory) {
